...
总分 = 行动得分
     + 已交付项目 × 300 − 失败交付 × 180
     + 流程纪律（按序 +1 / 错序 −1，逐点 25 分，计入区间 −12…+24）
     + 剩余资源（预算 × 0.28 + 剩余 Deadline × 42 + 稳定度 × 16）
     + 周五行交付奖励 400
```

剩余预算只是其中一项。按顺序留证据、减少返工和被打回，同样能明显拉开分数，不至于被“只是少花钱”压过去。

计分公式在 2026-09-14 做过一次权重调整。浏览器里旧的 `bestScore` 与新分数不可比，因此首次结算时会按新公式重置，之后正常比较。
...
CI（`.github/workflows/ci.yml`）会依次执行这三条命令。

当前自动测试覆盖固定种子路线、百万级 Token 与费用结算、模型能力差异、上下文压缩、卡牌冷却、操作顺序惩罚、模型切换、交付置信度、休整节点、价格数据、职业解锁，以及休整节点的上下文溢出保护、四类失败条件、终局计分构成、日志 id 确定性、手牌稳定性与旧存档迁移。

`npm run build` 会重新生成 `dist/`。本项目把 `dist/` 纳入版本库用于静态托管，因此改完源码后需要重新构建并一并提交，否则线上仍是旧包。
